 ...
 * @author fjli
 */
public class StandardTextEpgParser implements EpgParser {
	...
	@Override
	public EpgList parse(String file) throws IOException {
		try {
	...
	private EpgList parse0(String file, String charset) throws IOException {
		File filePath = new File(file);
		EpgList epgList = new EpgList();
		List<EpgItem> items = new ArrayList<>();
		epgList.setItems(items);
		epgList.setName(filePath.getName());
		BufferedReader reader = IOUtils.toBufferedReader(filePath, Charset.forName(charset));
		try {
			EpgItem current, previous = null;
			...
			while ((line = reader.readLine()) != null) {
				...
				if (!line.isEmpty()) {
					...
					if (m.find()) {
						...
						epgList.setName(line.substring(CHANNEL_NAME.length()).trim());
					}
				}
			}

...
function saveDelTaskSetting(dialog){
	var p = dialog.find("#settingForm").serialize();
	$.post('saveDelTaskSetting', p, function(data){
...
function showDelTaskConfigDialog(){
	var dialog = new window.top.TemplateDialog($("#dialog_setting"));

	dialog.click("#btnSave", function() {
		saveDelTaskSetting(dialog);
	});
	dialog.click("#btnCancel", function() {
		this.dialog.close();
	});

	top.$.get('loadDelTaskSetting', function(data){
		var height = 264;
		if(data != null){
...
function TL_DeleteTaskSetting(){
	showDelTaskConfigDialog();
}
